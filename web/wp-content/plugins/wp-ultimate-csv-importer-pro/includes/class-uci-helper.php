...
class SmackUCIHelper {
		...
		foreach ($uci_admin->getRowMapping() as $groupName => $groupValue) {
			switch ($groupName) {
				case 'CORE':
					...
					if ($this->getLastImportId() != '') {
						...
						if($mode != 'Schedule') {
							...
							} elseif($importType == 'WooCommerceVariations' || $importType == 'WooCommerceOrders' || $importType == 'WooCommerceCoupons'){
				...
				case 'YOASTSEO':
					...
					$yoastseoHelper->push_yoastseo_data();
					break;
				case 'TERMS':
		...
		if($importType != 'ticket') {
			switch ($importType) {
				...
				case 'Taxonomies':
					...
					break;
				case 'Comments':
					$returnArr = $helperObj->importComments();
					break;
				case 'WooCommerce':
                        ...
                        foreach ($data_array as $custom_key => $custom_value) {
                                ...
                                if( $importType != 'Users'){
                                        ...
                                        if( isset($core_serialize_info[$custom_key]) && $core_serialize_info[$custom_key] == 'on'){
                                                ...
                                                $get_meta_info = $wpdb->get_results($wpdb->prepare("select meta_key,meta_value from {$wpdb->prefix}postmeta where user_id=%d and meta_key=%s" , $pID , $custom_key ), ARRAY_A);
                                                if( !empty($get_meta_info)){
                                                        $wpdb->update($wpdb->prefix.'postmeta' , array('meta_value' => $custom_value ) , array('meta_key' => $custom_key , 'user_id' => $pID ));
                                                }else{
                                                        $wpdb->insert($wpdb->prefix.'postmeta' , array('meta_key'=> $custom_key , 'meta_value' => $custom_value , 'user_id' => $pID ));
                                                }
                                        }else{
	...
	public function convert_local_image_src($content, $post_id, $media_handle) {
		if(trim($content) != '') {
			...
			if ( ! empty( $searchNode ) ) {
				...
				/* preg_replace('/^<!DOCTYPE.+?>/', '', str_replace( array('<html>', '</html>', '<body>', '</body>'), array('', '', '', ''), $doc->saveHTML())); */
	...
	public function UCI_WPML_Supported_Posts ($data_array, $pId) {
		...
		if(empty($data_array['translated_post_title']) && !empty($data_array['language_code'])){
			$wpdb->insert( $wpdb->prefix.'icl_translations', array('language_code' => $data_array['language_code'],'element_id' => $pId , 'trid' => $trid + 1));
		}
		elseif(!empty($data_array['language_code']) && !empty($data_array['translated_post_title'])){
			...
			$wpdb->insert( $wpdb->prefix.'icl_translations', array( 'trid' => $trid_id, 'language_code' => $data_array['language_code'], 'source_language_code' => $translate_lcode ,'element_id' => $pId));
		}
	}
	...
	public function parse_media_settings($get_media_settings, $data_array) {
		...
		$endvalue = end($get_media_settings);
		$lastkey = key($get_media_settings);
		$count = preg_replace('/custom_height_/','',$lastkey);
		for($i=1;$i<=$count;$i++) {
			$custom_width = 'custom_width_'.$i.'';
			$custom_height = 'custom_height_'.$i.'';
			$custom_slug = 'custom_slug_'.$i.'';
			if(isset($get_media_settings[$custom_width]) && isset($get_media_settings[$custom_height]) && isset($get_media_settings[$custom_slug])) {
				$media_settings[$custom_slug] = $get_media_settings[$custom_slug];
				$media_settings[$custom_width] = $get_media_settings[$custom_width];
				$media_settings[$custom_height] = $get_media_settings[$custom_height];
			}
