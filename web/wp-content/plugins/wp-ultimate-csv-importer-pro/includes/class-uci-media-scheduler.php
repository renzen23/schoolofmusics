...
class SmackUCIMediaScheduler {
	...
	public static function convert_local_imageURL ($imageURL, $post_id, $renameimage = NULL, $media_settings = array()) {
		...
		if(isset($media_settings['thumbnail'])) {
			add_image_size( 'thumbnail', 0, 0, true );
		}
		...
		if(isset($media_settings['large'])) {
	        ...
	        if(isset($media_settings['custom'])) {
                        add_image_size( 'custom', 0, 0, true );
                }
                $endvalue = end($media_settings);
                $lastkey = key($media_settings);
                ...
                for($i=1;$i<=$count;$i++) {
                        ...
                        if(isset($media_settings[$custom_width]) && isset($media_settings[$custom_height]) && isset($media_settings[$custom_slug])) {
                               add_image_size( $media_settings[$custom_slug], $media_settings[$custom_width], $media_settings[$custom_height], true );
                        }
                }
		...
		if (!empty ($file)) {
			...
			if($rename_file_name != null) {
				...
				$wpdb->update($wpdb->posts, $attachment, array('ID', $attach_id));
