...
<div id="priority_template" class="panel wp_ultimate_csv_importer_pro" style="width: 99%; margin-top: 40px;">
 <form class="form-inline" id = "suggested_template" action="<?php echo esc_url(admin_url() . 'admin.php?page=sm-uci-import&step=suggested_template&eventkey=' . $_REQUEST['eventkey']);?>&mapping_type=advanced" method="post">
  <div class="col-md-10 col-md-offset-1 mt20" >
   <table class="table">
    <tr>
	     <input type="hidden" id="filename" value="<?php echo $_POST['uploaded_name']; ?>">
    ...
    if(!empty($template_order)) {
     ...
     <table class="table table-mapping font_suggestedtemp" id = "templates">
      ...
      <?php foreach($template_order as $templatename => $templateCount){
       $template_data = $wpdb->get_results($wpdb->prepare("select id, module, createdtime, mapping_type from wp_ultimate_csv_importer_mappingtemplate where templatename = %s", $templatename));
       ?>
       <tr>
        ...
        <td align="center">
         ...
         <a href="<?php echo esc_url(admin_url() . 'admin.php?page=sm-uci-import&step=mapping_config&eventkey='.$_REQUEST['eventkey'].'&templateid='.$template_data[0]->id); ?>&mapping_type=<?php echo $mapping_type; ?>" class="smack-btn smack-btn-info"><?php echo esc_html__('Use Template','wp-ultimate-csv-importer-pro');?></a>
         </td>
       </tr>
       <?php
       $count++;
...
<div class="clearfix"></div>
  ...
  <div class="pull-left">
   ...
   <div class="pull-right mb20"><a href="<?php echo esc_url(admin_url() . 'admin.php?page=sm-uci-import&step=mapping_config&eventkey='.sanitize_title($_REQUEST['eventkey'])); ?>" class="smack-btn smack-btn-primary btn-radius"><?php echo esc_html__('Create new Mapping','wp-ultimate-csv-importer-pro');?></a></div>
   </div>
