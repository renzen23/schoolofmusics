...
class SmackUCITypesDataImport {
	...
	public function push_types_data() {
		global $uci_admin;
		...
		foreach ($data_array as $key => $value) {
			if (array_key_exists($key, $types_fieldname)) {
				if ($types_fieldtype[$key] == 'date') {
					...
					if($type == 'users')
						update_user_meta($pID, $types_fieldname[$key], $tDate);
					else
						update_post_meta($pID, $types_fieldname[$key], $tDate);
				} elseif ($types_fieldtype[$key] == 'skype') {
					$tSkype['skypename'] = $value;
					if($type == 'users')
						update_user_meta($pID, $types_fieldname[$key], $tSkype);
					else
						update_post_meta($pID, $types_fieldname[$key], $tSkype);
					...
					if($type == 'users')
						update_user_meta($pID, $types_fieldname[$key], $value);
					else
						update_post_meta($pID, $types_fieldname[$key], $value);
				}
			}
		}
