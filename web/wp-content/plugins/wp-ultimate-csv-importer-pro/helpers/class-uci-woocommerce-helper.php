		...
		foreach ($data_array as $ekey => $eval) {
			switch ($ekey) {
				...
				case 'order_currency' :
					...
					break;
				case 'item_name' :
					$Item_metaDatas['order_item_name'] = $data_array[$ekey];
					break;
				case 'item_type' :
					$Item_metaDatas['order_item_type'] = $data_array[$ekey];
					break;
				case 'item_product_id' :
					$Item_metaDatas['_product_id'] = $data_array[$ekey];
					break;
				case 'item_variation_id' :
					$Item_metaDatas['_variation_id'] = $data_array[$ekey];
					break;
				case 'item_line_subtotal' :
					$Item_metaDatas['_line_subtotal'] = $data_array[$ekey];
					break;
				case 'item_line_subtotal_tax' :
					$Item_metaDatas['_line_subtotal_tax'] = $data_array[$ekey];
					break;
				case 'item_line_total' :
					$Item_metaDatas['_line_total'] = $data_array[$ekey];
					break;
				case 'item_line_tax' :
					$Item_metaDatas['_line_tax'] = $data_array[$ekey];
					break;
				case 'item_line_tax_data' :
					$Item_metaDatas['_line_tax_data'] = $data_array[$ekey];
					break;
				case 'item_tax_class' :
					$Item_metaDatas['_tax_class'] = $data_array[$ekey];
					break;
				case 'item_qty' :
					$Item_metaDatas['_qty'] = $data_array[$ekey];
					break;
				case 'fee_name' :
					$Fee_metaDatas['order_item_name'] = $data_array[$ekey];
					break;
				case 'fee_type' :
					$Fee_metaDatas['order_item_type'] = $data_array[$ekey];
					break;
				case 'fee_tax_class' :
					$Fee_metaDatas['_tax_class'] = $data_array[$ekey];
					break;
				case 'fee_line_total' :
					$Fee_metaDatas['_line_total'] = $data_array[$ekey];
					break;
				case 'fee_line_tax' :
					$Fee_metaDatas['_line_tax'] = $data_array[$ekey];
					break;
				case 'fee_line_tax_data' :
					$Fee_metaDatas['_line_tax_data'] = $data_array[$ekey];
					break;
				case 'fee_line_subtotal' :
					$Fee_metaDatas['_line_subtotal'] = $data_array[$ekey];
					break;
				case 'fee_line_subtotal_tax' :
					$Fee_metaDatas['_line_subtotal_tax'] = $data_array[$ekey];
					break;
				case 'shipment_name' :
					$Shipment_metaDatas['order_item_name'] = $data_array[$ekey];
					break;
				case 'shipment_method_id' :
					$Shipment_metaDatas['method_id'] = $data_array[$ekey];
					break;
				case 'shipment_cost' :
					$Shipment_metaDatas['cost'] = $data_array[$ekey];
					break;
				case 'shipment_taxes' :
					$Shipment_metaDatas['taxes'] = $data_array[$ekey];
					break;
		...
		if (!empty($attribute)) {
			...
			if(isset($attr_count)){
				for ($att = 0; $att < $attr_count; $att++) {
					$get_csvpro_settings = get_option('wpcsvprosettings');
					...
					$product_attributes[$attrlabel]['is_visible'] = 1;
					$product_attributes[$attrlabel]['is_variation'] = 1;
