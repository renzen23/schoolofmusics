...
class SmackUCIAIOSEODataImport {
	...
	public function push_aioseo_data() {
		global $uci_admin;
		$aioseodata = $uci_admin->getRowMapping();
		$data_array = $aioseodata['AIOSEO'];
		if( !empty($aioseodata)) {
			if(in_array('all-in-one-seo-pack/all_in_one_seo_pack.php', $uci_admin->get_active_plugins()) || in_array('all-in-one-seo-pack-pro/all_in_one_seo_pack.php', $uci_admin->get_active_plugins())) {//allinseo_customization
...
global $aioseoHelper;
$aioseoHelper = new SmackUCIAIOSEODataImport();
