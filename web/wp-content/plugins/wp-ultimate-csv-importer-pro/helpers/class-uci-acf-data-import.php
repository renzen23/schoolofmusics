...
class SmackUCIACFDataImport {
	 ...
	 *
	 */
	public function push_acf_data() {
		global $uci_admin;
		$data_array = $uci_admin->getRowMapping();
		if (!empty($data_array['ACF'])) {
			if (in_array('advanced-custom-fields-pro/acf.php', $uci_admin->get_active_plugins())) {
				$this->importDataForACFProFields($data_array['ACF'], $uci_admin->getImportAs(), $uci_admin->getLastImportId());
			} else {
				if (in_array('advanced-custom-fields/acf.php', $uci_admin->get_active_plugins())) {
					...
					if(is_dir($acf_pluginPath)) {
						...
						$this->importDataForACFFields($data_array['ACF'], $uci_admin->getImportAs(), $uci_admin->getLastImportId());
				}
			}
	...
	public function importDataForACFProFields($data_array, $importAs, $pID) {
		global $uci_admin;
			...
			foreach ($acf_field as $key => $value) {
				if (array_key_exists($value, $data_array)) {
					...
					if ($acf_type[$value] == 'user') {
						$userDet = $uci_admin->get_from_user_details($data_array[$value]);
						$data_array[$value] = $userDet['user_id'];
					}
					$createdFields[] = $value;
					if ($importAs == 'users') {
	...
	public function importDataForACFFields($data_array, $importAs, $pID) {
		global $wpdb;
		global $uci_admin;
		$acf_field = $acf_type = $createdFields = $acf_image_import_method = array();
		...
		foreach ($get_acf_fields as $acf_value) {
			...
			$acf_field[$get_acf_field['key']] = $get_acf_field['name'];
			$acf_type[$get_acf_field['key']] = $get_acf_field['type'];
			if ($get_acf_field['type'] == 'image') {
				$acf_image_import_method[$get_acf_field['key']] = $get_acf_field['save_format'];
			}
			if ($get_acf_field['type'] == 'file') {
				$acf_file_import_method[$get_acf_field['key']] = $get_acf_field['save_format'];
			}
			if ($get_acf_field['type'] == 'message') {
				$acf_field[$get_acf_field['key']] = $get_acf_field['key'];
				$acf_type[$get_acf_field['key']] = $get_acf_field['key'];
			}
		}
		foreach ($acf_field as $key => $value) {
			if (array_key_exists($value, $data_array)) {
				if (array_key_exists($value, $acf_type) && $acf_type[$value] == $value) {
					foreach ($get_acf_fields as $acf_value) {
						if($acf_value != '' && $acf_value != null) {
							$get_acf_field = @maybe_unserialize( $acf_value );
							if ( isset($get_acf_field['key']) && $get_acf_field['key'] == $value ) {
								$get_acf_field['message'] = $data_array[ $value ];
								$acf_msgid = $wpdb->get_col( $wpdb->prepare( "SELECT post_id from $wpdb->postmeta where meta_key = %s", $value ) );
								update_post_meta( $acf_msgid[0], $value, $get_acf_field );
							}
						}
					}
				}
				if ($acf_type[$key] == 'checkbox') {
					$data_array[$value] = explode(',', $data_array[$value]);
					$choices = $data_array[$value];
					$data_array[$value] = $choices;
				}
				if ($acf_type[$key] == 'image') {
					if ($acf_image_import_method[$key] == 'url') {
						#TODO image, file fields
						$data_array[$value] = $uci_admin->set_featureimage($data_array[$value], $pID);
					}
				} if($acf_type[$key] == 'file') {
					if ($acf_file_import_method[$key] == 'url') {
						#TODO image, file fields
						$data_array[$value] = $uci_admin->set_featureimage($data_array[$value], $pID);
					}
				}
				if ($acf_type[$key] == 'google_map') {
					$location = $data_array[$value];
					$lat_long = $uci_admin->get_latitude_longitude($location);
					$lat_long = explode(',', $lat_long);
					$latitude_point = isset($lat_long[0]) ? $lat_long[0] : '';
					$longitude_point = isset($lat_long[1]) ? $lat_long[1] : '';
					$map = array('address' => $location, 'lat' => $latitude_point, 'lng' => $longitude_point);
					$data_array[$value] = $map;
				}
				if ($acf_type[$key] == 'wysiwyg') {
					$fieldname = $data_array[$value];
					$data_array[$value] = $fieldname;
				}
				if ($acf_type[$key] == 'relationship' || $acf_type[$key] == 'taxonomy') {
					$relations = array();
					$get_relations = null;
					$get_relations = $data_array[$value];
					$exploded_relations = explode(',', $get_relations);
