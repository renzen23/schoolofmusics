...
jQuery( function() {
    jQuery( '#dwn_ftp_file' ).click(function() {
        jQuery.ajax({
            type: 'POST',
            url: ajaxurl,
            data: {
                'action': 'ftp_actions',
                'postdata': jQuery('#division2').find( 'input, select, textarea').serialize()
            },
            target:   '#targetLayer',
            xhr: function(){
                //upload Progress
            ...
            success: function (uploaded_file_info) {
                uploaded_file_info = JSON.parse(uploaded_file_info);
                if(uploaded_file_info['Success'] == 'Success!') {
                    document.getElementById('displaysection').style.display = "";
                    document.getElementById('division2').style.display = "none";
                    document.getElementById('file_name').value = uploaded_file_info['filename'];
                    var get_current_action = jQuery( '#form_import_file' ).attr( 'action' );
                    var get_file_extension = uploaded_file_info['filename'].split('/');
                    ...
                    if(fileextn == 'zip') {
                        jQuery.ajax({
                            ...
                            data: {
                                'action': 'upload_zipfile_handler',
                                'eventkey': uploaded_file_info['eventkey'],
                                'import_method': 'ftp',
                            },
                            success: function (zip_file_info) {
                                zip_file_info = JSON.parse(zip_file_info);
                                document.getElementById('choose_file').innerHTML = zip_file_info['data'];
                                jQuery('#modal_zip').modal('show');
                            }
                        });
                    } else {
                        document.getElementById('uploaded_name').value = uploaded_file_info['uploaded_name'];
                        document.getElementById('file_version').value = uploaded_file_info['version'];
                        jQuery("#filenamedisplay").empty();
                        jQuery('<label/>').text((uploaded_file_info['filename']) + ' - ' + uploaded_file_info['filesize']).appendTo('#filenamedisplay');
                    }
                    if(fileextn != 'zip') {
                        jQuery.ajax({
                            type: 'POST',
                            ...
                            data: {
                                'action': 'set_post_types',
                                'filekey': uploaded_file_info['eventkey'],
                                'uploadedname': uploaded_file_info['filename']
                            },
                            success: function (result) {
                                result = JSON.parse(result);
                                if(result != '') {
                                    if (result['is_template'] == 'yes') {
                                        var action = get_current_action + '&eventkey=' + uploaded_file_info['eventkey'];
                                    } else {
                                        var splitaction = get_current_action.split("&");
                                        var action = splitaction[0] + '&step=mapping_config&istemplate=no&eventkey=' + uploaded_file_info['eventkey'];
                                    }
                    ...
                    var warning = uploaded_file_info['Failure'];
...
function external_method() {
    ...
    if(external_url == ''){
        var warning = 'Filepath empty';
        swal("Warning!", "Please provide your external file URL.", "warning");
        return false;
    }
    var postdata = new Array({'external_file_url': external_url, 'import_method':'external_import'});
    document.getElementById('displaysection').style.display = "";
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if(data['Success'] == 'Success!') {
                ...
                var get_file = get_file_name.split('.');
                var fileextn = get_file[get_file.length-1];
                if(fileextn == 'zip' || fileextn == 'application/zip') {
                    jQuery.ajax({
                        type: 'POST',
                        url: ajaxurl,
                        data: {
...
/** Delete the Registered Fields **/
function DeleteCF(import_type,prefix,core_count,slug) {
    switch(slug) {
        case 'acf_pro_fields' :
            Delete_acf_pro_fields(prefix,import_type,core_count);
            break;
        case 'acf_fields' :
            Delete_acf_free_fields(prefix,import_type,core_count);
            break;
        case 'pods_custom_fields' :
            Delete_pods_fields(prefix,import_type,core_count);
            break;
        case 'types_custom_fields' :
            Delete_types_fields(prefix,import_type,core_count);
            break;
        default:
            Close_RegisterUI(prefix,core_count);
            break;
    }
}
...
function Delete_acf_pro_fields(prefix,import_type,core_count) {
    var field_name = document.getElementById(prefix + 'ui__CustomFieldName' + core_count).value;
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        async: false,
        data: {
            'action': 'delete_acf_pro_fields',
        ...
        success: function (response) {
            ...
            if(response == 'Field Deleted')
                swal('Success!', 'Field has been deleted', 'success')
            Close_RegisterUI(prefix,core_count);
        },
        error: function (errorThrown) {
            console.log(errorThrown);
...
function Validate_CF_types(id,prefix,mappingcount) {
    var field_type = jQuery('#'+ id + ' option:selected').val();
    switch(prefix) {
        case 'ACF' : {
            if(id = prefix + '_datatype_' + mappingcount)
                show_ACF_options(id,prefix,mappingcount);
            break;
        }
        case 'TYPES' : {
            if(field_type == 'select' || field_type == 'checkboxes' || field_type == 'radio') {
...
function show_PODS_relational_options(id,prefix,mappingcount,import_type) {
    var related_fieldId = prefix + '_relatedtype_' + +mappingcount;
    var i, j,bi_relation_options,data;
    /** Field_type is relational **/
    if(id == related_fieldId) {
        var related_to = jQuery('#' + id +" option:selected").val();
        /** If Relational Field is other than users,image,navigation for eg: Post,Page and so on **/
        if(related_to != 'custom' && related_to != 'user_role' && related_to != 'user_capability' && related_to != 'image_size' && related_to != 'navigation' && related_to != 'post_format' && related_to != 'post_status' && related_to != 'country' && related_to != 'us_states' && related_to != 'calendar_week' && related_to != 'calendar_year') {
            jQuery('#'+prefix+'_CDOfd'+mappingcount).hide();
            jQuery('#'+prefix+'_bidirecfd'+mappingcount).show();
            /** Need to show the related fields **/
            jQuery.ajax({
                type: 'POST',
                url: ajaxurl,
                async: false,
                datatype:'JSON',
                data: {
                    'action': 'add_bidirectional_fields',
                    'import_type': import_type,
                    'related_to' : related_to,
                },
                success: function (response) {
                    data = JSON.parse(response);
                    bi_relation_options = document.getElementById(prefix+"_bidirec"+mappingcount);
                    /** Remove Already exists options in dropdown **/
                    for(i=1;i<bi_relation_options.length;i++){
                        bi_relation_options.remove(i);
                    }
                    /** End **/
                    /** Add the bi-relational field title dynamically **/
                    if(data.length != 0){
        ...
        /** If Relational Field is Custom Type **/
        else if(related_to == 'custom') {
    ...
    /** Field Type is other than relational field **/
    else {
...
function show_ACF_options(id,prefix,mappingcount) {
    var field_type  = jQuery("#" + id + "  option:selected").val();
    if(field_type == 'checkbox' || field_type == 'radio button' || field_type == 'select') {
        jQuery('#'+prefix+'_trchoice'+mappingcount).show();
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if(prefix == 'CORECUSTFIELDS'){
                ...
                jQuery('#' + prefix + '_check_' + core_count).addClass('RegField_iCheck');
                jQuery('input.RegField_iCheck').iCheck({
                    checkboxClass: 'icheckbox_square-green',
                    radioClass: 'iradio_square-green',
                    increaseArea: '20%' // optional
                });
...
/**
 * It Is used to call Custom Fields functions and display the second UI of registration
 **/
function RegisterCF(prefix,slug, import_type, mappingcount) {
    var validateField = validateCF(prefix, mappingcount, slug);
    if(validateField) {
        if(confirm("Register the custom fields...?") == true) {
            switch(slug) {
                case 'acf_pro_fields' :
                    Register_acf_pro_fields(prefix, slug, import_type, mappingcount);
                    break;
                case 'acf_fields' :
                    Register_acf_free_fields(prefix, slug, import_type, mappingcount);
                    break;
                case 'pods_custom_fields' :
                    Register_pods_fields(prefix, slug, import_type, mappingcount);
                    break;
                case 'types_custom_fields' :
                    Register_types_fields(prefix, slug, import_type, mappingcount);
                    break;
            }
        }
        else {
            swal('Success!', 'You have cancelled the field registration.', 'success')
            Close_RegisterUI(prefix,mappingcount);
        }
    }
}
...
/**
 * It is used to insert the ACF Pro Custom Fields in database
 * mappingcount and core_count both have same value
 **/
function Register_acf_pro_fields(prefix,slug,import_type,mappingcount) {
    var field_data = Field_information(prefix,slug);
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: {
            'action': 'register_acfpro_fields',
            'prefix': prefix,
            'slug': slug,
        ...
        success: function (data) {
            console.log(data);
            if(data == 'ACF Fields are already registered') {
                document.getElementById(prefix+'newrow'+mappingcount).style.display = 'none';
                swal('Warning!', 'You have a field with same name already.', 'warning')
            }
            else {
                jQuery('#' + prefix + '_num_' + mappingcount).attr('name', prefix + '_check_' + field_data.name);
                jQuery('#' + prefix + '_tdc_count' + mappingcount).css('display', '');
                jQuery('#' + prefix + '__CustomField' + mappingcount).val(field_data.label);
                jQuery('#' + prefix + 'CustomField' + mappingcount).html('[Name: ' + field_data.name + ']');
                document.getElementById(prefix + mappingcount + '_CustomField').innerHTML = '<b>' + field_data.label + '</b>';
                jQuery('#' + prefix + '__fieldname' + mappingcount).val(field_data.name);
                Show_RegisteredUI(prefix, slug, mappingcount);
                jQuery('#' + prefix + '_check_' + mappingcount).addClass('RegField_iCheck');
                jQuery('input.RegField_iCheck').iCheck({
                    checkboxClass: 'icheckbox_square-green',
                    radioClass: 'iradio_square-green',
                    increaseArea: '20%' // optional
...
function Register_acf_free_fields(prefix,slug,import_type,mappingcount) {
    var field_data = Field_information(prefix,slug);
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
...
function Register_pods_fields(prefix,slug,import_type,mappingcount) {
    var field_data = Field_information(prefix,slug);
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        ...
        success: function (data) {
            console.log(data);
            if(data == 'PODS Fields are already registered') {
                document.getElementById(prefix+'newrow'+mappingcount).style.display = 'none';
                swal('Warning!', 'You have a field with same name already.', 'warning')
            }
            else {
...
function Register_types_fields(prefix,slug,import_type,mappingcount) {
    var field_data = Field_information(prefix,slug);
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        ...
        success: function (data) {
            console.log(data);
            if(data == 'TYPES Fields are already registered') {
                document.getElementById(prefix+'newrow'+mappingcount).style.display = 'none';
                swal('Warning!', 'You have a field with same name already.', 'warning')
            }
            else {
...
/**
 * It is used to return the field information
 **/
function Field_information(prefix,slug) {
    var core_count = document.getElementById(slug+'_count').value;
    var field_info = {};
    field_info['label'] = document.getElementById(prefix + 'ui__CustomFieldLabel' + core_count).value;
    field_info['name'] = document.getElementById(prefix + 'ui__CustomFieldName' + core_count).value;
    field_info['desc'] = document.getElementById(prefix + 'ui__CustomFieldDesc' + core_count).value;
    field_info['field_type'] = document.getElementById(prefix + '_datatype_' + core_count).value;
    field_info['required'] = document.getElementById(prefix + 'ui__CustomFieldOption' + core_count).checked;
    /** Change space in field name to underscore **/
    field_info['name'] = field_info['name'].toLowerCase();
    field_info['name'] = field_info['name'].replace(/\s/g, "_").replace(/\./g,"_");
    /** End **/
    if(prefix === 'ACF') {
        if(field_info['field_type'] == 'checkbox' || field_info['field_type'] === 'radio button' || field_info['field_type'] === 'select')
            field_info['choice'] = jQuery('#' + prefix + '_type_options' + core_count).val();
        if(field_info['field_type'] == 'user')
            field_info['role'] = jQuery('#' + prefix + '_role' + core_count).val();
    }
    if(prefix === 'PODS') {
        if(field_info['field_type'] == 'relationship') {
            field_info['related_to'] = document.getElementById(prefix+'_relatedtype_'+core_count).value;
            field_info['bi_directional'] = document.getElementById(prefix+'_bidirec'+core_count).value;
        }
    }
    if(prefix === 'TYPES') {
        if(field_info['field_type'] == 'select' || field_info['field_type'] == 'checkboxes' || field_info['field_type'] == 'radio')
            field_info['choice'] = jQuery('#' + prefix + '_type_options' + core_count).val();
        if(field_info['field_type'] == 'checkbox')
            field_info['choice'] = jQuery('#' + prefix + 'chck_op' + core_count).val();
    }
    return field_info;
}
...
function update_template(eventkey) {
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        data: {
        ...
        success: function (data) {
            data = JSON.parse(data);
            if(data['notification'] == 'Updated Successfully') {
                swal("Success!", data['notification'], "success")
                var currentURL = location.protocol + '//' + location.host + location.pathname + '?page=sm-uci-managers&step=templatemanager';
...
function show_savetemplate() {
    if(document.getElementById('save_template_chkbox').checked){
        jQuery('#templatename').slideUp(300);
    }
    else{
        jQuery('#templatename').SlideDown(300);
    }
}
...
function filter_template( template_call ){
    var table_row_count = 0;
    var template_row_count = 0;
    var filename = document.getElementById("filename").value;
    var start_date = jQuery('#from-date').val();
    var end_date = jQuery('#to-date').val();
    var search = jQuery('#search').val();
    var limit = jQuery('#template_limit').val();
    var template_table = document.getElementById('templates');
    if((start_date == '' && end_date == '' && search == '') || template_table == null) {
        return false;
    }
    if(template_call === undefined){
        template_table.innerHTML = '';
        jQuery('#template_offset').val('0');
        jQuery('#template_row_count').val('0');
    }
    var offset = jQuery('#template_offset').val();
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        async: false,
        data: {
            'action': 'filter_template',
        ...
        success: function (data) {
            jQuery('#template_row_count').val(offset);
            offset = parseInt(offset) + parseInt(limit);
            jQuery('#template_offset').val(offset);
            if(data == 'Count Exceeded.'){
                return false;
            }
            if(data) {
                var template_list = JSON.parse(data);
    ...
    jQuery('#scroll_template').scroll(function() {
        var from_scroll = 0;
        var table_row_count = jQuery('#templates tr').length;
        var template_row_count = parseInt(jQuery('#template_row_count').val());
        if(table_row_count >= template_row_count){
            return false;
        }
        if(jQuery(this).scrollTop() + jQuery(this).innerHeight() + parseInt(1) >= this.scrollHeight) {
            filter_template(from_scroll);
        }
    });
}
...
function edit_schedule() {
    ...
    var time = jQuery('#timetoschedule option:selected').val();
    var frequency = jQuery('#schedule_frequency option:selected').text();
...
function igniteSchedule() {
    var get_import_configuration = getImportConfiguration();

    if(jQuery("#schedule").is(':checked')) {
        var date = jQuery("#datetoschedule").val();
        var time = jQuery("#timetoschedule option:selected").text();
        var frequency = jQuery("#schedule_frequency option:selected").text();
        var eventkey =  document.getElementById('eventkey').value;
        jQuery.ajax({
            type: 'POST',
            ...
            async: false,
            data: {
                'action': 'schedule_your_current_event',
                'eventkey': eventkey,
                'configData': get_import_configuration,
                'date':date,
                'time':time,
                'frequency':frequency,
            },
            success: function (response) {
                ...
                var warning = responseData['notification'];
                swal("Warning!", warning, "warning")
                if(responseData['notification'] != undefined) {
                    var currentURL = location.protocol + '//' + location.host + location.pathname + '?page=sm-uci-import';
                    swal({
                            title: warning,
...
function validateDateIntervals(){
    var start_date = document.getElementById('postdatefrom').value;
    var end_date = document.getElementById('postdateto').value;
    ...
    if(start_date_limit > end_date_limit){
        ...
        var warning = 'End date must be greater than start date';
        swal("Warning!", warning, "warning")
    }
}
...
function show_updateTemplate(filename,id) {
    var version = jQuery('#record_'+id +' option:selected').text();
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        async: false,
        ...
        success: function (data) {
            ...
            var currentURL = location.protocol + '//' + location.host + location.pathname + '?page=sm-uci-import&step=mapping_config&eventkey=' + event_data['eventkey'] + '&templateid=' + event_data['id'];
            window.location.assign(currentURL);
        },
        error: function (errorThrown) {
            console.log(errorThrown);
...
function selectrevision(id,filename) {
    var revision = jQuery('#record_'+ id + ' option:selected').text();
    jQuery(document).ready(function(){
        var inserted =jQuery('#inserted_'+ revision).text();
        var updated = jQuery('#updated_'+ revision).text();
        var deleted = jQuery('#skipped_'+ revision).text();
        var eventdate = jQuery('#eventdate_'+ revision).text();
    });
    if(revision != '--Select--') {
        jQuery.ajax({
            type: 'POST',
            ...
            success: function (data) {
                ...
                var created = jQuery("#inserted_"+id).text(datas.created);
                var updated = jQuery("#updated_"+id).text(datas.updated);
                var skipped = jQuery("#skipped_"+revision).text(datas.skipped);
                var eventdate = jQuery("#eventdate_"+id).text(datas.event_started_at);
                console.log(datas);
            },
            error: function (errorThrown) {
                console.log(errorThrown);
...
function downloadFile(id , filename) {
    ...
    if(revision != '--Select--') {
        jQuery.ajax({
            ...
            success: function (data) {
                jQuery("#file_download_url").attr("href", data); //"http://www.w3schools.com/jquery");
                console.log(data);
            },
            error: function (errorThrown) {
                console.log(errorThrown);
        ...
        swal('Warning!', 'Choose any revision to download!.', 'warning')
    }
    window.location = jQuery("#file_download_url").attr("href");
}


function delete_file_event(id,filename) {
    var path = jQuery('#record_'+ id + ' option:selected').val();
    var version = jQuery('#record_'+ id + ' option:selected').text();
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        ...
        success: function (data) {
            console.log(data);
            data = JSON.parse(data);
            if(data['notice'] == 'File was scheduled.') {
                swal('Warning!', "Can't delete. You have configured the schedule event with the same file.", 'warning')
            }
            else {
                swal('Success!', data['notice'], 'success')
                window.location.reload();
            }
        },
        error: function (errorThrown) {
...
function delete_scheduled_file(data) {
    document.getElementById('alert_box').style.display = '';
    jQuery( "#confirm_notice" ).dialog({
        modal: true,
        buttons: {
            "Ok": function() {
                ...
                jQuery.ajax({
                    ...
                    success: function (response) {
                        ...
                        swal('Warning!', response['notice'], 'warning')
                        window.location.reload();
                    },
                    error: function (errorThrown) {
                    },
            ...
            Cancel: function() {
                jQuery( this ).dialog( "close" );
                document.getElementById('alert_box').style.display = '';
                swal('Warning!', 'File can not be delete', 'warning')
            }
        }
    });
}

function delete_record_event(id,importas,filename,module) {
    var version = jQuery('#record_'+ id + ' option:selected').text();
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
...
function delete_all_event(id,filename,module) {
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        async: false,
...
function delete_scheduled_file1(data,id) {
    document.getElementById('alert_box').style.display = '';
    jQuery( "#confirm_notice" ).dialog({
        modal: true,
        buttons: {
            "Ok": function() {
                ...
                jQuery.ajax({
                    ...
                    async: false,
                    data: {
                        'action': 'deleteAllScheduledEvent',
                        'schedule_idList': data['scheduleList'],
                        'file_id': id,
                    },
                    success: function (response) {
                        console.log(response);
            ...
            Cancel: function() {
                jQuery( this ).dialog( "close" );
                document.getElementById('alert_box').style.display = '';
                swal('Warning!', 'Files can not be delete', 'warning')
            }
        }
    });
}

function trash_records(id,module,filename,record_status) {
    var path = jQuery('#record_'+ id + ' option:selected').val();
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        ...
        success: function (data) {
            data = JSON.parse(data);
            if(data['notice'] == "Records Not Found") {
                swal('Warning!', data['notice'], 'warning')
            } else {
                swal('Success!', data['notice'], 'success')
            }
            jQuery(document).ready(function(){
                if (record_status =="trash") {
                    jQuery("#restore_"+id).show();
                    jQuery("#trash_"+id).hide();
                }
                else {
                    jQuery("#trash_"+id).show();
                    jQuery("#restore_"+id).hide();
                }});
...
function downloadAll_files(id) {
    jQuery.ajax({
        type: 'POST',
        url: ajaxurl,
        async: false,
        data: {
            'action': 'download_AllFiles',
            'id': id,
        },
        success: function (data) {
            jQuery("#file_download_url").attr("href", data);
        },
        error: function (errorThrown) {
            console.log(errorThrown);
        }
    ...
    for(var i=1;i<=4;i++) {
        if(parseInt(id) == parseInt(i)) {
            ...
            if(parseInt(i) == 1) {
                ...
                document.getElementById('rightside_content').style.height = '450px';
                document.getElementById('import_holder').style.height = '450px';
...
function databaseoptimization() {
    ...
    if(!orphaned && !unassignedTags && !postpagerevisions && !autodraftedpostpage && !postpagetrash && !spamcomments && !trashedcomments && !unapprovedcomments && !pingbackcomments && !trackbackcomments) {
        swal('Warning!', 'Please select any option before run your optimisation.', 'warning')
    }
    jQuery.ajax({
        url: ajaxurl,
        type: 'post',
        data: {
            'action': 'database_optimization_process',
            'orphaned': orphaned,
        ...
        success: function (response) {
            ...
            if(response['orphaned'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['orphaned'] + ' ) '+uci_importer.db_orphanedMsg + '</p>';
            if(response['unassignedTags'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['unassignedTags'] + ' ) '+ uci_importer.db_tagMsg + '</p>';
            if(response['postpagerevisions'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['postpagerevisions'] + ' ) '+ uci_importer.db_revisionMsg + '</p>';
            if(response['autodraftedpostpage'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['autodraftedpostpage'] + ' ) ' + uci_importer.db_draftMSg + '</p>';
            if(response['postpagetrash'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['postpagetrash'] + ' ) '+ uci_importer.db_trashMsg + '</p>';
            if(response['spamcomments'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['spamcomments'] + ' ) ' + uci_importer.db_spamMsg + '</p>';
            if(response['trashedcomments'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['trashedcomments'] + ' ) ' + uci_importer.db_commentTrashMsg + '</p>';
            if(response['unapprovedcomments'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['unapprovedcomments'] + ' ) ' + uci_importer.db_unapprovedMsg + '</p>';
            if(response['pingbackcomments'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['pingbackcomments'] + ' ) ' + uci_importer.db_pingbackMsg + '</p>';
            if(response['trackbackcomments'] != 'non_affected')
                document.getElementById('log').innerHTML += '<p style="margin:15px; margin-left:10px;"> ( ' + response['trackbackcomments'] + ' ) '+ uci_importer.db_trackbackMsg + '</p>';
        }
    });
}
...
jQuery(function(){
    jQuery('.media_seo').on('ifChecked', function(){
        var key = jQuery(this).data('key');
        jQuery('#img_seo_mapping_for_' + key).prop('disabled', false);
        jQuery('.selectpicker').selectpicker('refresh');
    });

    jQuery('.media_seo').on('ifUnchecked', function(){
        var key = jQuery(this).data('key');
        jQuery('#img_seo_mapping_for_' + key).prop('disabled', true);
        jQuery('.selectpicker').selectpicker('refresh');
    });
...
jQuery(function () {
    jQuery("#image-handling-btn").click(function () {
        if (jQuery(this).is(":checked")) {
            jQuery("#image-handling-btn-opt").show();
        } else {
            jQuery("#image-handling-btn-opt").hide();
        }
    });
});
...
jQuery(function(){
    jQuery('#custom-size-add').hide();
    jQuery('#media_custom_size').on('ifChecked', function(){
        jQuery('#custom-size-add').show();
    });

    jQuery('#media_custom_size').on('ifUnchecked', function(){
        jQuery('#custom-size-add').hide();
    });
});
...
function GetDynamicTextBox(value) {
    return  '<td style="width:21%;"><input name = "custom_slug_'+value+'" type="text" id="media_custom_size_slug_'+value+'" class="custom_media"  placeholder="Slug" /></td>' +
        '<td style="width:21%;"><input name = "custom_width_'+value+'" type="text" id="media_custom_size_width_'+value+'" class="custom_media"  placeholder="Width" /></td>' +
        '<td style="width:21%;"><input name = "custom_height_'+value+'" type="text" id="media_custom_size_height_'+value+'" class="custom_media" placeholder="Height" /></td>'+
        '<td  class="text-danger remove"  name ="custom_delete_'+value+'" id = "custom_delete_'+value+'" style="font-size:20px;cursor:pointer;"><i class="icon-trash4"></i></td>'
}
...
jQuery(function(){
    jQuery('#save_template_chkbox').on('ifChecked', function(){
        jQuery('#templatename').show("slow");
    });

    jQuery('#save_template_chkbox').on('ifUnchecked', function(){
        jQuery('#templatename').hide("slow");
    });
});
...
function redirect_manager(id) {
    ...
    var get_step = getUrlParameter('step');
    var step = '';
    if(id == 1) {
        step = 'filemanager';
    } else if(id == 2) {
        step = 'schedulemanager';
    } else if(id == 3) {
        step = 'templatemanager';
    } else if(id == 5) {
        step = 'logmanager';
    }
    if(get_step === undefined) {
        window.location.replace(url + '&step=' + step);
    } else {
        window.location.replace(url + "&step=" + step);
    }
}
