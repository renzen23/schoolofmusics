...
class SmackUCIScheduleManager
{
    ...
    public function editSchedule($schedule_data) {
        ...
        $update_schedule = $wpdb->update('wp_ultimate_csv_importer_scheduled_import',array('scheduledtimetorun' => $schedule_data['time'],'scheduleddate' => $schedule_data['date'], 'frequency' => $frequency ), array('id' => $schedule_data['id']));
        if($update_schedule){
	...
	//print_r($_POST);die;
        ...
        $storeSchedule = $wpdb->insert('wp_ultimate_csv_importer_scheduled_import',
            array('templateid' => $templateId,
                  ...
                  'scheduleddate' => sanitize_text_field($_POST['date']),
    ...
    public static function doSchedule($data) {
        ...
        for ($i = $startLimit; $i <= $endLimit; $i++) {
            ...
            $uci_admin->importData($eventKey, $module, $import_method, $import_mode, $recordList[$startLimit], $i, $eventMapping, null, $mediaHandling, $importConfig);
