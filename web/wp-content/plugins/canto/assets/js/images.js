					...
					if (self.state.src == targetUrl) {
						...
						if(currentCount == length) {
							...
							if(found > (self.state.start+self.state.limit))
								jQuery('#canto_fbc_loadMore').show();
							else
								jQuery('#canto_fbc_loadMore').hide();
						}
					}
        ...
        return (
			<div class="grid">
				...
				<ul className="attachments" id="__attachments-view-fbc">
					...
					<div id="canto_fbc_loadMore_wrap">
						<button className="btn" id="canto_fbc_loadMore" onClick={this.loadMore}>Load More</button>
					</div>
	            </ul>
			</div>
        );
