...
class canto_media {
	...
	function media_upload_canto( $errors ) {
		...
		if ( get_option( 'fbc_app_expire_token' ) < time() || ( get_option( 'fbc_flight_domain' ) == '' || get_option( 'fbc_app_token' ) == '' ) ) {
			...
			if ( get_option( 'fbc_app_expire_token' ) < time() ) {
				echo '<form><h3 class="media-title"><span style="font-size:14px;font-family:Helvetica,Arial">' .
				     __( "<strong>Oops!</strong> Your security token has expired Please re-authenticate your Canto account. <a href=\"javascript:;\" onclick=\"window.top.location.href='"
				         . get_bloginfo( 'url' ) . "/wp-admin/options-general.php?page=canto_settings'\">Plugin Settings</a>", 'canto' ) . '</span></h3></form>';
			}

			if ( get_option( 'fbc_flight_domain' ) == '' || get_option( 'fbc_app_token' ) == '' ) {
				echo '<form><h3 class="media-title"><span style="font-size:14px;font-family:Helvetica,Arial">' .
				     __( "<strong>Oops!</strong> You haven't connected your Canto account yet. <a href=\"javascript:;\" onclick=\"window.top.location.href='"
				         . get_bloginfo( 'url' ) . "/wp-admin/options-general.php?page=canto_settings'\">Plugin Settings</a>", 'canto' ) . '</span></h3></form>';
			}
			...
			?>
            <div class="canto-class">
                <img src="<?php echo CANTO_FBC_URL; ?>/assets/loader_white.gif" id="loader">
			...
			wp_register_script( 'fbc-react-vendor', CANTO_FBC_URL . 'public/assets/app.vendor.bundle.js', array(), false, true );
			wp_register_script( 'fbc-react-bundle', CANTO_FBC_URL . 'public/assets/app.bundle.js', array(), false, true );
			...
			wp_enqueue_style( 'canto-style', CANTO_FBC_URL . 'public/assets/app.styles.css' );
			?>
		...
		$get_paged = sanitize_text_field( $_GET['paged'] );

		$get_paged = isset( $get_paged ) ? intval( $get_paged ) : 0;
		if ( $get_paged < 1 ) {
			$get_paged = 1;
		}
		$start = ( $get_paged - 1 ) * 10;
		if ( $start < 1 ) {
			$start = 0;
		}
